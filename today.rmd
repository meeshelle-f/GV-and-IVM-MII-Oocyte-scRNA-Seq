...
```{R, plot medians of chr8 13 and 21}
temp1<- read.csv("~/GitHub/scSeq/GSE158802/p_1_temp1_10.6.21", row.names=1)
...
```{R, IT WORKS USE THIS ONE for p and q!!}
...
for(i in 1:length(U)){ #had to alter by length to get code to work 
  ...
  p=ggplot(data = matrix, aes(x = CHR_ARM, y = Z.score)) +
  geom_point(stat = "identity", size=.1) + labs(x = "Chromosome", y = "Z score") 
  #instead of geom_point ^ can use: geom_dotplot(binaxis='y',stackdir='center',dotsize=.5,width=10) 
 ...
 #  myplot=
      p+ theme_classic()+ theme(axis.text.x = element_blank(),axis.ticks.x = element_blank(), strip.text.x=element_text(size=10)) +ggtitle(title_name)+ 
    facet_grid(~CHR_ARM, scales='free_x',space='free_x',switch = 'x')+
  #+facet_wrap(~CHR_ARM, scales='free_x')+
       
      geom_hline(data=data_hline, aes(yintercept=hline),color="red",size=1) + geom_text(data=data_hline,aes(0,4,label=round(hline,digits=1),fontface="bold.italic"),size=3)
                                                                          
   
    # Save plots to tiff. Makes a separate file for each plot.
    file_name = paste(title_name, ".tiff", sep="")
    png(file_name,width=600, heigh=600)
    print(plot_list[[i]])
    dev.off() }
